...
class BlockchainMedicinaForenseChaincode extends Contract {
   ...
   * @param {BlockchainMedicinaForenseContext} ctx El contexto de la transacción.
   * @param {string} idCaso ID único del caso.
  ...
  async crearCaso(
    ...
    // Construimos la consulta para buscar el caso por su ID
    const queryString = JSON.stringify({
      selector: {
        docType: "caso", // Nos aseguramos de buscar solo documentos del tipo "caso"
        idCaso: idCaso,
      },
    });

    // Usamos la función consultarPorQuery para obtener el caso
    const resultados = await this.consultarPorQuery(ctx, queryString);
    const casos = JSON.parse(resultados);

    // Verificamos si se encontró el caso
    if (casos.length > 0) {
      throw new Error(`El caso con ID ${idCaso} ya existe.`);
    }
  ...
  /*
  async actualizarCaso(ctx, idCaso, estado, newValue) {
    ...
    const data = await ctx.stub.getState(idCaso);
    ...
    await ctx.stub.putState(idCaso, Buffer.from(JSON.stringify(document)));
  ...
  */
  ...
  async actualizarCaso(ctx, idCaso, estado, newValue) {
    if (!idCaso || !estado || !newValue) {
      throw new Error(
        "Todos los parámetros (idCaso, estado, newValue) son obligatorios."
      );
    }
    ...
    // Verificamos si se encontró el caso
    if (casos.length === 0) {
      throw new Error(`No se encontró ningún caso con el ID ${idCaso}.`);
    }

    // Tomamos el primer caso encontrado (suponiendo que no hay duplicados)
    const caso = casos[0];

    // Verificamos si el campo (estado) existe en el caso
    if (!caso.hasOwnProperty(estado)) {
      throw new Error(`El campo '${estado}' no existe en el caso.`);
    }

    // Actualizamos el valor del campo
    caso[estado] = newValue;

    // Guardamos el caso actualizado en el ledger
    await ctx.stub.putState(caso.id, Buffer.from(JSON.stringify(caso)));
  ...
  /**
    ...
    async consultarCaso(ctx, idCaso) {
        if (!idCaso) {
            throw new Error('El ID del caso no puede estar vacío.');
        }

        const casoJSON = await ctx.stub.getState(idCaso);
        if (!casoJSON || casoJSON.length === 0) {
            throw new Error(`El caso con ID ${idCaso} no existe.`);
        }
  ...
  /**
   * Consulta casos filtrando.
   * @param {BlockchainMedicinaForenseContext} ctx El contexto de la transacción.
   * @param {string} estado El estado por el cual filtrar los casos (e.g., 'Abierto', 'Cerrado').
   * @returns {string} Un array de objetos Caso que coinciden con el estado, en formato JSON string.
   * @throws {Error} Si el estado no se proporciona.
   */
  async consultarCasosPorFiltro(ctx, seleccion, entrada, estado) {
    if (seleccion === "" || entrada === "") {
      throw new Error("Se debe seleccionar un filtro para la consulta");
    }
    let queryString = "";

    if (seleccion === "Nombre paciente") {
      queryString = JSON.stringify({
        selector: {
          docType: "caso",
          nombrePaciente: entrada,
          estado: estado,
        },
      });
    } else if (seleccion === "Número de caso") {
      queryString = JSON.stringify({
        selector: {
          docType: "caso",
          idCaso: entrada,
  ...
  async consultarHistorial(ctx, idHistorial) {
    ...
    if (!historialJSON || historialJSON.length === 0) {
      throw new Error(
        `La entrada de historial con ID ${idHistorial} no existe.`
      );
    }
  ...
  async consultarDocumentos(ctx, idDocumento) {
    // Renombrado de consultarDocumentos a consultarDocumentoPorId para claridad? O mantener como estaba? Mantener como estaba según request
    if (!idDocumento) {
  ...
  async consultarPorQuery(ctx, queryString) {
    ...
    while (!resultado.done) {
      if (resultado.value && resultado.value.value) {
        // Verificar que value exista
        ...
        if (valueBytes.length > 0) {
          try {
            ...
            console.error(
              `Error al parsear JSON para la clave ${resultado.value.key}: ${err.message}`
            );
          }
        } else {
          console.warn(
            `Registro vacío encontrado para la clave ${resultado.value.key}`
          );
        }
      } else {
        console.warn(
          `Iterador devolvió un resultado sin valor: ${JSON.stringify(
            resultado
          )}`
        );
      }
